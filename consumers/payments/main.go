...
// 支付消费者服务入口
// 模拟 Kafka 消费者组 "payments-group"，消费支付 topic 的 3 个 partition
// 每个 partition 对应一个独立的 HTTP 服务端口，可独立查询状态和消息
func main() {
	// 命令行参数：每个 partition 对应一个端口
	portP0 := flag.Int("port-p0", 8088, "Partition 0 的 HTTP 服务端口")
	portP1 := flag.Int("port-p1", 8089, "Partition 1 的 HTTP 服务端口")
	portP2 := flag.Int("port-p2", 8090, "Partition 2 的 HTTP 服务端口")
	broker := flag.String("broker", "http://localhost:8081", "Broker 服务地址")
	groupID := flag.String("group-id", "payments-group", "消费者组 ID")
	dbDir := flag.String("db-dir", "./data", "数据库存储目录")
	flag.Parse()

	fmt.Println("=== 支付消费者服务启动 ===")
	fmt.Printf("Broker: %s, GroupID: %s, DB目录: %s\n", *broker, *groupID, *dbDir)

	// 创建支付消息存储（线程安全，所有 partition 共享同一个 store）
	store, err := database.NewPaymentsStore(*dbDir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "创建 PaymentsStore 失败: %v\n", err)
		os.Exit(1)
	}

	// 创建消费者服务，配置 topic 和轮询间隔
	cs := consumer.NewConsumerService("payments", *groupID, "payments", *broker, *dbDir, 2*time.Second)

	// 为 3 个 partition 分别创建监听器，共享同一个 PaymentsStore
	...
	for i, port := range ports {
		...
		go func(s *http.Server, p int, a string) {
			fmt.Printf("[payments] Partition %d HTTP 服务启动: %s\n", p, a)
			if err := s.ListenAndServe(); err != nil && err != http.ErrServerClosed {
				fmt.Fprintf(os.Stderr, "[payments] Partition %d HTTP 服务错误: %v\n", p, err)
			}
		}(server, pid, addr)
	}

	fmt.Println("[payments] 所有 partition 监听器和 HTTP 服务已启动")
	fmt.Printf("[payments] 端口映射: P0=%d, P1=%d, P2=%d\n", *portP0, *portP1, *portP2)
	...
	fmt.Println("\n[payments] 收到退出信号，正在优雅关闭...")
	cancel()
	fmt.Println("[payments] 支付消费者服务已停止")
}
